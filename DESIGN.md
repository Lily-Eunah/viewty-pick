...
> 기준 문서: `README.md` · 버전 3.6 (2026-06-11) · 대상: MVP (제품 50~100개)
...
> v3.6 변경 요약: 올리브영 큐레이터 활동 가이드 확인 → "기타 개인이 운영하는 온라인 채널" 허용 + "비교 분석·큐레이션" 권장 콘텐츠라 **자체 사이트 적격**으로 상향(§12.3·§18). 제휴 공통 요건으로 **단순 링크 나열 금지 → 실질 콘텐츠 필수** 명문화(§12.2).
...
| 올리브영 큐레이터 | ✅ 활동 가이드에 **"기타 개인이 운영하는 온라인 채널"** 명시 허용, **"비교 분석·추천 리스트(큐레이션)"이 권장 콘텐츠** | 자체 운영 사이트 적격. ⚠️ 단 금지 콘텐츠 **"단순 링크 나열·반복 게시"** 주의 → 각 페이지에 실질 콘텐츠(리뷰·비교 분석·큐레이션 근거) + 광고 고지 필수(§11 씬 콘텐츠 회피와 동일). 별도 문의 없이 적격 판단, 확인은 선택 |
...
**핵심 결론**: 링크 수동 입력으로 자동화 장벽은 해소됐다. 남은 "안 될 이유"는 **약관의 게재 허용 범위** 하나다 — 쿠팡은 웹 게재가 명시 허용돼 확실하고, **네이버 쇼핑 커넥트는 본인 채널 등록형·사전 심사 없음이라 자체 사이트 등록 가능성이 높으며**, **올리브영은 활동 가이드가 "기타 개인이 운영하는 온라인 채널"을 허용하고 "비교 분석·큐레이션"을 권장 콘텐츠로 명시해 자체 사이트도 적격**(단 단순 링크 나열 금지 → 실질 콘텐츠 + 고지 필요), 지그재그·에이블리는 약관이 "본인 SNS"를 전제로 해 **독립 가격비교 사이트 게재 가능 여부만 각 사에 확인**하면 된다. 확인 후 허용되면 **수수료 순(네이버·지그재그·올리브영 우선)**으로 연동하고, 불허 판매처는 **가격 비교에만 노출하고 제휴 수익은 제외**한다. (모든 페이지에 제휴 수수료 고지 문구 노출 — 공통 필수)
...
- **콘텐츠 적격성(제휴 공통 요건)**: 제휴사 약관(특히 올리브영)이 "단순 링크 나열"을 금지하므로, 모든 제품/큐레이션 페이지는 **가격표 외에 실질 콘텐츠**(비교 분석, 추천 근거, 사용 팁, ml당 가격 해설 등)를 갖춘다. SEO 씬 콘텐츠 회피(§11)와 동일 요건 → 추가 작업 없음.
...
- [x] **올리브영 큐레이터**: 활동 가이드 확인 완료 — "기타 개인이 운영하는 온라인 채널" 허용 + "비교 분석·큐레이션"이 권장 콘텐츠라 **적격**. 조건: 페이지마다 실질 콘텐츠(단순 링크 나열 금지) + 광고 고지. (확실히 하려면 고객센터 확인은 선택)
