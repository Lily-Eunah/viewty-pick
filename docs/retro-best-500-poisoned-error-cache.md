...
| 07-07 02:5x | 리디자인(PR #101) 배포 8b000be0 → `/best` 200 확인 |
| 07-07 04:0x | **크롤 없이 재발** — 배포 정확히 1시간 후. "크롤 부하" 가설 붕괴 |
| 07-07 04:3x | revalidate 값 상관관계 발견(3600s 페이지만 사망) → open-next.config 검사 → **WORKER_SELF_REFERENCE 바인딩 누락 확정** (어댑터 소스에서 throw 라인 확인) |
| 07-07 06:16 | PR #102 (서비스 바인딩 추가) → 배포 b9193572 → T+80분 감시 시작 |
| 07-07 07:37 | **T+81분: /best·/best/[slug] 전부 200 — 첫 자연 만료(60분) 생존. 픽스 최종 검증 ✅** (이전엔 모든 배포가 T+60분에 사망) |

## 9. 진짜 마지막 반전 — 세 번째 근본 원인

PR #99 배포 다음 날, `/best`가 **크롤 없이** 또 죽었다. 배포 후 정확히 1시간 —
`revalidate = 3600`의 첫 자연 만료 시점이었다.

### 결정적 상관관계

| 페이지 | revalidate | 상태 |
|---|---|---|
| `/best`, `/best/[slug]` | 3600 (1시간) | 배포 1시간 후 **500** |
| 홈, `/c/*`, `/skin/*` | 86400 (24시간) | 정상 |

### 근본 원인: 처음부터 ISR 재검증은 한 번도 동작한 적이 없었다

OpenNext Cloudflare의 ISR 큐(memory-queue)는 만료된 페이지를 **Worker가 자기 자신을
service binding으로 fetch**해서 재렌더한다. 어댑터 소스:
...
이 바인딩이 캐시 셋업(PR #93) 때부터 wrangler.jsonc에 누락 → **시간 기반 ISR 재검증이
프로덕션에서 단 한 번도 성공한 적이 없었다.** 그동안 들키지 않은 이유:

- 홈·카테고리(86400s)는 자연 만료 전에 **매일 크롤 purge가 시계를 리셋** → 영원히 안 걸림
- `/best`(3600s)만 유일하게 자연 만료를 겪는 페이지 → 매일 혼자 죽음
- 크롤은 근본 원인이 아니라 **"1시간 뒤 고장"을 매일 아침으로 스케줄링해주던 시계**였다
- 수동 revalidate가 "치유"처럼 보인 것도 엔트리를 fresh로 만들어 만료를 1시간 미룬 것뿐

### 왜 앞의 두 픽스로 안 잡혔나

- PR #98/#99의 방어(쿼리 재시도·페이지 가드)는 **앱 코드 안**의 예외만 막는다. 이 throw는
  OpenNext의 stale-serve 기계 내부, 앱 코드 바깥에서 난다.
- crawl.yml warmup은 크롤 직후(전부 fresh)에 체크 → 1시간 뒤의 고장을 볼 수 없었다.
- 로컬 workerd 재현이 실패한 이유: purge→렌더는 **포그라운드 경로**(정상)이고, 고장난 건
  자연 만료의 **백그라운드 재검증 경로**. 로컬에서 1시간을 기다린 적이 없었다.

### 픽스 (PR #102) — 한 줄

```jsonc
"services": [{ "binding": "WORKER_SELF_REFERENCE", "service": "viewtypick" }]
```

### 최종 교훈

11. **"매일 같은 시각"과 "이벤트 후 정확히 N분"은 다른 단서다** — 전자는 cron을, 후자는
    타이머(TTL/만료)를 가리킨다. 이번 장애의 진짜 시그니처는 "크롤 후"가 아니라 "만료 후 1시간"이었다.
12. **인프라 어댑터의 필수 바인딩은 셋업 시점에 체크리스트로** — 어댑터는 조용히 던지고
    (IgnorableError), 프레임워크는 삼키고, 증상은 엉뚱한 곳에서 났다.
13. **N차 근본 원인을 의심하라** — fs 폴백(1차), 에러 캐시 박제(2차)도 실재하는 결함이었지만,
    그 아래에 "재검증이 원래 안 됨"(3차)이 있었다. 각 픽스가 증상을 바꿔가며 다음 층을 드러냈다.
