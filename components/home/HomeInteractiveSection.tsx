  ...
  return (
    <>
      ...
      <section className="px-4 py-3 bg-bg flex flex-col gap-3">
        <h3 className="text-[14px] font-black text-title tracking-tight">인기 카테고리</h3>
        <div className="grid grid-cols-2 gap-2.5">
          {[
            { href: '/c/sunscreen', label: '☀️ 선크림' },
            { href: '/c/toner',     label: '💧 토너'   },
            { href: '/c/cream',     label: '🧴 크림'   },
            { href: '/c/serum',     label: '✨ 세럼'   },
          ].map(({ href, label }) => (
            <Link
              key={href}
              href={href}
            ...
            >
              <span className="text-[14px] font-extrabold text-title">{label}</span>
      ...
      {/* TOP Carousel (hidden while skin filter active) */}
